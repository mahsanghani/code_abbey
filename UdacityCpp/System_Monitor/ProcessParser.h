...
/* function goal: get CPU percentage by acquiring relevant times of active
 * occupation of CPU*/
std::string ProcessParser::getCpuPercent(std::string pid) {
  ...
  float freq =
      sysconf(_SC_CLK_TCK); // the number of clock ticks per second,
                            // specifically, the kernel is configured for how
                            // many clocks per second (or Hz of clock).
...
std::vector<std::string> ProcessParser::getPidList() {
  ...
  // Note: it is IMPORTANT that for each element (file || dirctoary) inside the
  // /proc
  while (dirent *dirp = readdir(dir)) {
    ...
    /* Note:
     * The function starting with "[]" is called "lambda function" or "arrow
     * function", here, it specifically checks from start to end, and check each
     * element is a digit (return true if yes for each) in the end, if all the
     * elements are digits, then the all_of() returns true
     */
  ...
  while (std::getline(stream, line)) {
    if (line.compare(0, name.size(), name) == 0) {
      ...
      return values[2]; // For example:
                        ...
                        // (buildd@lgw01-amd64-054) ....... (omitted a lot
                        // tailing stuff)
    }
  }
...
std::string ProcessParser::getOSName() {
  ...
  while (std::getline(stream, line)) {
    if (line.compare(0, name.size(), name) == 0) {
      ...
      found++; // pinpoint the position of "=", and access the next position of
               // it
