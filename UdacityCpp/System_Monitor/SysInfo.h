...
// Note: it contains a lot of error checking logics, which means it's a complex
// getter
std::vector<std::string> SysInfo::getCoresStats() const {
  ...
  for (int i = 0; i < this->coresStats.size(); i++) {
    ...
    temp +=
        Util::getProgressBar(this->coresStats[i]); // It will ultimatedly gives
                                                   // you info plus bars like:
                                                   //  "cpu 0: |||||||||||"
